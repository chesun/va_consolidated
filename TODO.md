...
- [ ] **[Phase 1c §5.4]** Replace `cap log close _all` + unnamed `log using` + `log close` triplet with named-log pattern across all ~110 relocated .do files (`cap log close <name>` / `log using "...smcl", replace text name(<name>)` / `log close <name>`). Current pattern is predecessor-inherited: every nested .do file's `cap log close _all` closes the MASTER log too, which means main.do's log capture stops after the FIRST nested .do invocation. Discovered 2026-05-17 when investigating r(603) in `clean_acs_census_tract.do`: master log was only 7,445 bytes (captured up to first `cap log close _all`); main.do's residual source echo after the error leaked into the nested-do's per-file log instead. Mechanical sweep — same shape as the 2026-05-17 dual sweep helper. Named logs co-exist properly: master stays open throughout the run, captures every phase banner + every `do do/...` invocation + error propagation; per-file nested logs sit alongside without conflict.
