...
[LEARN:tooling] **SSH ControlMaster pattern for Scribe rsync.** No 2FA on Scribe means single auth per session; ControlMaster keeps the auth warm so multiple rsync calls within ~4 hours don't re-prompt. Config in `~/.ssh/config`: `Host scribe` block with `ControlMaster auto`, `ControlPath ~/.ssh/cm_socket/%r@%h:%p`, `ControlPersist 4h`. One `mkdir ~/.ssh/cm_socket` per machine. Once authenticated via `ssh scribe`, all subsequent `rsync`/`scp`/`ssh` calls reuse the channel.
