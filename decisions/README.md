...
| [0028](0028_accept-thin-variant-va-cells-soft-check.md) | accept thin per-spec VA cells in restricted-variant samples (subsamples drop students post-merge; CFR shrinks them; weight-only) + downgrade check_va_estimates per-spec count assert from hard-halt to soft; resolves e968d13 Phase-7 VA FAIL | 2026-06-09 | Superseded in part by #0033 | Specification |
...
| [0031](0031_git-workflow-prevents-manual-data-egress.md) | data-security rationale for the git-on-Scribe workflow: gitignored data means `git pull` brings down only code, so git can't accidentally copy restricted data onto a local machine the way FileZilla drag-and-drop can (a real, recurring lab incident); refines ADR-0022, affirms git as the offboarding workflow recommendation while leaving the successor's choice open; lab-guide input | 2026-06-13 | Decided | Infrastructure |
| [0032](0032_staffqoi98-extended-scale-relax-clamp-and-check.md) | staffqoi98 is coded on an extended scale where -3 = "severe problem" (staffqoiclean*.do; only -3-coded item); relax the ADR-0027 imputation clamp floor to -3 for it (imputation.do climatevars loop) + widen check_survey_indices SUB-CHECK 1 min bound to [-3.01,0] for it; resolves the 2026-06-13 full-run Phase-7 survey FAIL; not an index component so no paper-output impact; amends ADR-0027, extends ADR-0028 pattern | 2026-06-20 | Decided | Methodology |
| [0033](0033_remove-heuristic-checks-without-hard-basis.md) | remove Phase-7 data-checks with no hard basis in the data: z-index tail bounds + 3 correlation soft-signals + VA centered-mean + VA SD envelope + CFR cell-floor soft (was ADR-0028) + age/cohort_size soft ranges; loosen source-item & raw-index ranges to the hard coding bound (drop min≤0/max≥0 centering); add one structural presence check to check_va_estimates; VA numeric correctness moves to the M4 golden master. Triggered by z_climateindex min=-7.09 false-halt on the 2026-06-20 run | 2026-06-21 | Decided | Methodology |
