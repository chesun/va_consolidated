...
- [ ] **For relocated/new do files specifically (per ADR-0021):** (a) header description block present (purpose / invoked-from / conventions / references); (b) one-liner present in `do/main.do` at the invocation site, cross-checked against the relocated script's own header; (c) every `save` / `export` / `outsheet` / `esttab using` / `graph export` / `outreg2 using` / `texsave` / `translate` / `log using` targets a CANONICAL global from `do/settings.do` (no LEGACY-path writes — verify with `grep -nE 'save\|export\|esttab using\|graph export\|outsheet\|outreg2 using\|texsave\|^\s*translate \|log using'`); (d) **for each LEGACY `include` or `do` of a predecessor `.doh`, trace every `$<global>` referenced inside the included file** (`grep -nE '\$[a-zA-Z_][a-zA-Z0-9_]*' <legacy-doh>`) and confirm the global is bound in `do/settings.do` OR aliased before the include (e.g., `global projdir "$caschls_projdir"` if the .doh expects `$projdir`). See `MEMORY.md [LEARN:stata]` 2026-04-30 for the canonical pattern; surfaced in coder-critic round-1 BLOCK on `do/sibling_xwalk/siblingoutxwalk.do` 2026-04-30. **`translate` + `log using` added 2026-05-08** after coder-critic round-1 BLOCK on `do/survey_va/factor.do:131` — sed-mistranslated `translate` line slipped past pre-commit because the original grep pattern omitted log-write commands; see `quality_reports/reviews/2026-05-08_step-7-survey-va_coder_review.md`.
- [ ] **`/*` balance (per 2026-05-17 sweep).** Every modified `.do` / `.doh` file must satisfy `grep -c '/\*' <file>` equals `grep -c '\*/' <file>`. An imbalance indicates a path-glob `*` was written inside a comment (e.g., `prepare/*`), which Stata's parser greedily counts as a `/*` open and creates a runaway block comment that silently hides downstream code. Use `<x>` (or `<file>` / `<filename>`) as the placeholder instead. See `.claude/rules/stata-code-conventions.md` "Wildcards in comments" and `quality_reports/plans/2026-05-17_comment-bug-sweep.md` v3 for the bug analysis.
- [ ] **Log path mirror (per 2026-05-17 sweep).** If the file is at `do/<reldir>/<name>.do`, every `log using` and `translate` reference points at `$logdir/<reldir>/<name>.{smcl,log}` — i.e., nested under `$logdir/` mirroring the do/ structure. The `cap mkdir "$logdir"` block also has one `cap mkdir "$logdir/<each-intermediate-path>"` line per parent directory. Top-level `do/main.do` and `do/settings.do` are exempt. See `.claude/rules/stata-code-conventions.md` "Per-file logging structure".
