/*------------------------------------------------------------------------------
do/samples/create_va_g11_out_sample.doh — g11 OUTCOME-VA sample (base; byte-eq to v1)
================================================================================

PURPOSE
    Build the in-memory g11 OUTCOME-VA sample dataset.  **BYTE-IDENTICAL
    to `create_va_g11_out_sample_v1.doh' in the predecessor** — likely a
    pre-v1/v2-split historical name retained when the explicit `_v1' was
    added.

    Functionality identical to v1 (per ADR-0009 v1 is canonical; base is
    just an unsuffixed alias).  Same restriction (g11, CAASPP, outcome-
    year range, `diff_school_prop' >= 0.95) and same prior-score wiring
    (calls `create_prior_scores_v1.doh').
...
RELOCATION HISTORY (per ADR-0021, applied 2026-04-30)
    Source:      cde_va_project_fork/do_files/sbac/create_va_g11_out_sample.doh
    Destination: do/samples/create_va_g11_out_sample.doh
