...
OUTPUTS (CANONICAL per ADR-0021 sandbox; verified via grep on file body)
    $logdir/va_var_explain.smcl (via log using)
    $logdir/va_var_explain.smcl + $logdir/va_var_explain.log (translate)
...
* CANONICAL: cd removed; relocated paths now absolute (per [LEARN:workflow] absolute-after-cd batch 2c).
 ...
 log using "$logdir/va_var_explain.smcl", replace text
...
foreach outcome in enr enr_2year enr_4year {
 ...
 log close
 translate $logdir/va_var_explain.smcl $logdir/va_var_explain.log, replace
