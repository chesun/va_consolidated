...
PURPOSE
    clean CDE staff-school FTE assignments per year; produces yearly staffschoolfte dtas + tempfile consumed by clean_sch_char.
...
OUTPUTS (CANONICAL per ADR-0021 sandbox; verified via grep on file body)
    ...
    $logdir/clean_staffschoolfte.smcl (via log using)
    $logdir/clean_staffschoolfte.smcl + $logdir/clean_staffschoolfte.log
...
log using "$logdir/clean_staffschoolfte.smcl", replace text
...
timer list
log close
translate $logdir/clean_staffschoolfte.smcl $logdir/clean_staffschoolfte.log, replace
