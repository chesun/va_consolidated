...
OUTPUTS (CANONICAL per ADR-0021 sandbox; verified via grep on the file body)
    ...
    $logdir/va_corr_schl_char_fig.smcl + .log
...
log using "$logdir/va_corr_schl_char_fig.smcl", replace text
...
cap log close
cap translate "$logdir/va_corr_schl_char_fig.smcl" ///
    "$logdir/va_corr_schl_char_fig.log", replace
