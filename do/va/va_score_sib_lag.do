/*------------------------------------------------------------------------------
do/va/va_score_sib_lag.do — Phase 1a §3.3 step 3 batch 3d relocation
================================================================================

PURPOSE
    Sibling-lag forecast-bias diagnostic for score VA. Estimate VA with lag-1 older-sibling outcomes as controls + lag-2 older-sibling outcomes as FB leave-out var. Per do_all.do: "kept active for diagnostic; not reported in the paper but kept available in case coauthors revisit."
...
OUTPUTS (CANONICAL per ADR-0021 sandbox)
    $estimates_dir/va_cfr_all_v[12]/{vam,spec_test,fb_test,va_est_dta}/<...>_s_sp_sib1_ct[_sib2_lv].{ster,dta}
    $logdir/va/va_score_sib_lag.smcl + .log

RELOCATION (per plan v3 §3.3 step 3 batch 3d, applied 2026-05-08)
    Source: cde_va_project_fork/do_files/sbac/va_score_sib_lag.do
...
********************************************************************************
/* run score VA estimation with lag 1 older sibling as controls and lag 2 older sibling
...
do $vaprojdir/do_files/sbac/va_score_sib_lag.do
 */
...
cap log close va_score_sib_lag

log using "$logdir/va/va_score_sib_lag.smcl", replace text name(va_score_sib_lag)

di as text _n "{hline 80}"
di as text "va_score_sib_lag.do — RUN START: `c(current_date)' `c(current_time)'"
di as text "{hline 80}"
...
foreach version in v1 v2 {
  ...
  foreach subject in ela math {
    use $datadir_clean/va_samples_`version'/score_s if touse_g11_`subject'==1 & touse_sib_lag==1, clear
    ...
    // dropped before score_s is saved, because merge_sib.doh restricts its merge
    // to keepusing(touse, sibling)-pattern vars.  This sibling-lag diagnostic and
    // va_out_sib_lag.do are their only consumers, so re-merge them here from the
    ...
    vam sbac_`subject'_z_score ///
      ...
      estimates($estimates_dir/va_cfr_all_`version'/vam/va_`subject'_s_sp_sib1_ct.ster, replace)
    ...
    estimates save $estimates_dir/va_cfr_all_`version'/spec_test/spec_`subject'_s_sp_sib1_ct.ster, replace
    ...
    label data "`subject' test score VA estimates for sibling sample with lag 1 older sibling controls"
    compress
    save $estimates_dir/va_cfr_all_`version'/va_est_dta/va_`subject'_s_sp_sib1_ct.dta, replace
  }
...
foreach version in v1 v2 {
  ...
  foreach subject in ela math {
    ...
    vam sbac_`subject'_z_score ///
      ...
      data(merge tv score_r) ///
      driftlimit(`score_drift_limit') ///
      estimates($estimates_dir/va_cfr_all_`version'/vam/va_`subject'_s_sp_sib1_ct_sib2_lv.ster, replace)
    ...
    ******** Forecast bias test: Regress predicted scores on value added
    di " Forecast bias test leave out var: lag 2 older sibling outcome; sample: sibling; `subject' VA specification: lag 1 older sibling as control"
    ...
    estimates save $estimates_dir/va_cfr_all_`version'/fb_test/fb_`subject'_s_sp_sib1_ct_sib2_lv.ster, replace
...
cap log close va_score_sib_lag
cap translate "$logdir/va/va_score_sib_lag.smcl" ///
  "$logdir/va/va_score_sib_lag.log", replace
